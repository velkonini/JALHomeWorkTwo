public class MyArr {
    static int getArr(String[][]myArray) throws MyDataExeption, MySizeExeption {
        if(myArray.length == 4 && myArray[0].length == 4) {
            System.out.println("Длинна массива верная");
        } else {
            throw new MySizeExeption("Неправильная длинна массива");
        }
        ...
        for (int i=0;i<myArray.length;i++) {
            ...
            for(int j=0;j<myArray[i].length;j++) {
                try {
                    ...
                    throw new MyDataExeption("Не является числом "+i+" "+j);
                }
            }
        }
